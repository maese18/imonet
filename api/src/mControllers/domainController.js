...
class MongoDomainController {
	...
	insert = (req, res, next) => {
		...
		console.log('insertObject:', JSON.stringify(insertObjectOrObjects, null, 2));

		if (Array.isArray(insertObjectOrObjects)) {
			this.insertMany(req, res, next);
		} else {
			insertObjectOrObjects.createdAt = new Date();
			insertObjectOrObjects.tenantId = req.app.locals.tenantId;
			console.log('insertObject:', JSON.stringify(insertObjectOrObjects, null, 2));
			this.mongoDbAdapter.db
				...
				.then(result => {
					if (result.insertedCount === 1) {
						let meta = { message: `inserted 1 ${collection} item` };
						console.log('result.ops=', JSON.stringify(result.ops, null, 2));
						sendResponse(req, res, next, HttpStatusCodes.Created, 'data', result.ops[0], meta);
					} else {
	...
	insertMany = (req, res, next) => {
		...
		this.mongoDbAdapter.db
			...
			.then(result => {
				if (result.insertedCount >= 1) {
					let meta = { message: `inserted ${result.ops.length} ${collection} items` };

					sendResponse(req, res, next, HttpStatusCodes.Created, 'data', result.ops, meta);
				} else {
					throw new IllegalArgumentException(TAG, 'Undefined exception when inserting document');
				}
			})
	...
	updateOne = (req, res, next) => {
		...
		this.mongoDbAdapter.db
			...
			//.replaceOne({ _id: ObjectID(req.params.id) }, { $set: updateObject, $unset: { files: '' }, $currentDate: { lastModified: true } })
			.updateOne({ _id: ObjectID(req.params.id) }, { $set: updateObject, $currentDate: { lastModified: true } })
			.then(result => {
				...
				this.mongoDbAdapter.db
					...
					.then(result => {
						...
						let meta = { message: `Updated item ${id} of collection ${collection}` };
						sendResponse(req, res, next, HttpStatusCodes.Ok, 'data', result, meta);
		...
		try {
			this.mongoDbAdapter.db
				...
				.then(result => {
					console.log(`Found ${JSON.stringify(result)}`);
					if (result) {
						let meta = { message: `Found item ${id} of collection ${collection}` };
						sendResponse(req, res, next, HttpStatusCodes.Ok, 'data', result, meta);
					} else {
	...
	findAll = (req, res, next) => {
		...
		if (attributes) {
			let projects = {};
			attributes.forEach(attribute => (projects[attribute] = 1));
		...
		cursor
			.toArray()
			.then(result => {
				let meta = { message: `Found ${result.length} items of collection ${collection}` };
				sendResponse(req, res, next, HttpStatusCodes.Ok, 'data', result, meta);
	...
	finByQuery = ({ query = {}, sort = {}, project = {}, req, res, next }) => {
		let collection = req.params.collection;
